...
class _signupState extends State<signup> {
                  ...
                  children: [
                    SizedBox(
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          if (emailcntoraler.text.isNotEmpty &&
                              passcntoraler.text.isNotEmpty &&
                              phonecntoraler.text.isNotEmpty &&
                              namecntoraler.text.isNotEmpty) {
                            try {
                              UserCredential userCrendetial =
                                  await _auth.createUserWithEmailAndPassword(
                                      email: emailcntoraler.text,
                                      password: passcntoraler.text);
                              userCrendetial.user!
                                  .updateDisplayName(namecntoraler.text);
  ...
  dosignup(int f, String username, String useremail, String userpass,
      String userphone, dropdownValue) async {
    String userpalce = dropdownValue;
    print(userpalce);
    if (f == 0) {
      var res = await fetch
          .usersignup(username, useremail, userpass, userphone, userpalce)
          .then((res) {
        if (res == null) {
          print("Duplication");
          AlertDialog alert = const AlertDialog(
            content: Text(
                "this email is used before it\n هذا البريد مستخدم من قبل "),
          );
      ...
      var res = await fetch
          .sellersignup(username, useremail, userpass, userphone, userpalce)
          .then((res) {
        if (res == null) {
          print("Duplication");
          AlertDialog alert = const AlertDialog(
      ...
      var res = await fetch
          .deliverysignup(username, useremail, userpass, userphone, userpalce)
          .then((res) {
        if (res == null) {
          print("Duplication");
          AlertDialog alert = const AlertDialog(
